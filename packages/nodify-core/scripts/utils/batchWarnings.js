          ...
          warnings.forEach(warning => {
            ...
            if (id) {
              const loc = warning.loc
                ? `${relativeId(id)}: (${warning.loc.line}:${warning.loc
                    .column})`
                : relativeId(id);
  ...
  NAMESPACE_CONFLICT: {
    ...
    fn: warnings => {
      ...
      warnings.forEach(warning => {
        stderr(
          `${chalk.bold(
            relativeId(warning.reexporter)
          )} re-exports '${warning.name}' from both ${relativeId(
            warning.sources[0]
      ...
      nestedByPlugin.forEach(({ key: plugin, items }) => {
        ...
        nestedByMessage.forEach(({ key: message, items }) => {
          ...
          items.forEach(warning => {
            ...
            const loc = warning.loc
              ? `${relativeId(warning.id)}: (${warning.loc.line}:${warning.loc
                  .column})`
...
function showTruncatedWarnings(warnings) {
  ...
  sliced.forEach(({ key: id, items }) => {
    ...
    if (items.length > 1) {
      stderr(
        `...and ${items.length - 1} other ${items.length > 2
          ? 'occurrences'
          : 'occurrence'}`
      );
    }
